...
class BatchNormalization(Layer):
    ...
    def forward(
        ...
        self.input_var = (input ** 2).mean(r_axes, keepdims=True) - self.input_mean ** 2
        ...
        v = T.where(deterministic, self.avg_var, self.input_var) + self.const
        output = nn.normalize(input, mean=m, variance=v, epsilon=0)
        return W * output + b
