...
def piecewise_constant(init, values, step=None):
    ...
    if step is None:
        step = tensor.Variable(
            0, trainable=False, name='PiecewiseConstant_step')
...
class PiecewiseConstant(Schedule):
    ...
    def __init__(self, init, values):
        ...
        self.value = piecewise_constant(self.init, self.values,
                                        self.step)[0]
...
def ExponentialMovingAverage(value, alpha, step=None, init=None):
    if step is None:
        _step = tensor.Variable(
            0, trainable=False, name='step', dtype='float32')
    else:
        _step = step
    if init is None:
