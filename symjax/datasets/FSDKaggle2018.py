...
class FSDKaggle2018:
    ...
    def load(path=None):
        ...
        meta_train = np.loadtxt(io.BytesIO(f.read('FSDKaggle2018.meta/train_post_competition.csv')),
                          delimiter=',', skiprows=1, dtype='str')
        meta_test = np.loadtxt(io.BytesIO(f.read('FSDKaggle2018.meta/test_post_competition_scoring_clips.csv')),
                          delimiter=',', skiprows=1, dtype='str')
