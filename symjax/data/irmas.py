...
class irmas:
    ...
    def download(path):
        ...
        if not os.path.exists(path + 'irmas/IRMAS-TrainingData.zip'):
            ...
            urllib.request.urlretrieve(
                url, path + 'irmas/IRMAS-TrainingData.zip')
    ...
    def load(path=None):
        ...
        train_labels = np.array([Id[unique_cat.index(cat)]
                                 for cat in train_labels]).astype('int32')
        test_labels = np.array([Id[unique_cat.index(cat)]
                                for cat in test_labels]).astype('int32')
